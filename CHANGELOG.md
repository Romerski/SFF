...
- Search filter now caches its DLC / soundtrack / non-Windows drop decisions per app id, so re-running the same query (or any query that returns overlapping app ids) skips the structural reclassification and the per-item DEBUG log line both. Earlier builds re-walked the metadata on every search and emitted one debug line per filtered Hubcap row, which produced 100+ noisy lines on a populated franchise query like "resident evil" plus a redundant GetItems round trip when all the candidates were already in `_STEAM_PLATFORM_CACHE`. The classification is a pure function of the cached metadata so the decision cache is consistent for the process lifetime; a single summary line at DEBUG carries up to 5 sample drops per reason for triage. Re-searches add nothing to the log
...
- New global Settings entry "Check for game updates" plus a per-game override map (`UPDATE_CHECK_OVERRIDES`) and an interval setting (`UPDATE_CHECK_INTERVAL_MIN`, default 60). Defaults to OFF — opting in via the Settings panel or per-tile toggle is what enables the periodic sweep, so a fresh install never bursts a wave of `appdetails` + `Login Anonymous` calls on startup. When ON, a periodic timer walks every installed game once per interval, gates on the global setting and the per-game override, and dispatches at most one Steam CM check per game per interval. Cross-game dispatches are paced one per 2 seconds
